...
def online_fp8_requant_per_channel(weight_in_out):
    """bf16 dense weight [in, out] -> (e4m3 weight [in, out], f32 scale [out]).

    Per-output-channel: scale = (max |w| over the input axis) / 448, so the
    largest magnitude in each output column maps to the top of the e4m3 range.
    ...
    return w_fp8, jnp.squeeze(scale, axis=0)
