...
def f32_ln_forward(self, x):
    ...
    return F.layer_norm(x.to(torch.float32), self.normalized_shape, w, b, self.eps).to(od)
...
for name, mod in (("bf16", ve_a), ("bf16+f32LN", ve_b)):
    ...
    outs[name] = {k: torch.from_numpy(np.asarray(v.jax()).astype(np.float32)) for k, v in o.items()}
...
for name, arm in outs.items():
    ...
    for k in ref:
        a = ref[k].float().numpy()[0][valid]; b = arm[k].float().numpy()[0][valid]
        d = np.abs(a - b); rel = d.mean() / (np.abs(a).mean() + 1e-12)
...
for K in ("S3_ln2", "S7_proj"):
    for name, arm in outs.items():
        ...
        print(f"{K} {name:<12} per-token maxerr p50={np.percentile(pt,50):.4f} "
