...
LAYERS = [
    ("gate_up", (64, 512), (64, 512), (None, "model"), (None, "model"), ("model", )),
    ("down", (256, 64), (256, 64), ("model", None), ("model", None), (None, )),
    ("q", (64, 4, 32), (64, 128), (None, "model", None), (None, "model"), ("model", )),
    ("o", (4, 32, 64), (128, 64), ("model", None, None), ("model", None), (None, )),
]
...
def _stub_and_import():
    ...
    for k in [k for k in sys.modules
              if k.split(".")[0] in ("tpu_inference", "torch", "torchax", "safetensors", "vllm")]:
        del sys.modules[k]
    ...
    torch = mod("torch", Tensor=type("Tensor", (), {}), uint8="uint8", uint16="uint16", uint32="uint32")
    torch.nn = types.SimpleNamespace(Module=object)
    mod("torchax")
    mod("safetensors", safe_open=None)
    mod("vllm").__path__ = []
    mod("vllm.config", ModelConfig=type("ModelConfig", (), {}), VllmConfig=type("VllmConfig", (), {}),
        get_current_vllm_config=lambda: None)
    mod("vllm.model_executor").__path__ = []
    mod("vllm.model_executor.model_loader", register_model_loader=lambda name: (lambda cls: cls)).__path__ = []
    mod("vllm.model_executor.model_loader.dummy_loader", DummyModelLoader=type("DummyModelLoader", (), {}))
    mod("vllm.model_executor.models").__path__ = []
    mod("vllm.model_executor.models.utils", AutoWeightsLoader=type("AutoWeightsLoader", (), {}))
    mod("tpu_inference").__path__ = [str(ROOT / "tpu_inference")]
    mod("tpu_inference.utils", t2j=lambda t, use_dlpack=False: t, get_mesh_shape_product=lambda mesh, axis: 1)
    mod("tpu_inference.logger",
        init_logger=lambda *a, **k: type("L", (), {"__getattr__": lambda s, n: (lambda *a, **k: None)})())
...
def _measure():
    ...
    def phys_of(a, on):
        ...
        return int(np.prod(a.sharding.shard_shape(a.shape))) * a.dtype.itemsize * len(devs)
    ...
    class Block(nnx.Module):
        def __init__(self):
            for name, shape, _, spec, _, _ in LAYERS:
                setattr(self, name, nnx.Param(jnp.zeros(shape, jnp.bfloat16), out_sharding=spec))
    ...
    def abstract_model():
        with jax.set_mesh(mesh):
            return nnx.eval_shape(Model)  # ShapeDtypeStruct params, as model_loader builds them
    ...
    def checkpoint(li, i):
        ...
        with jax.set_mesh(host_mesh):
            return jnp.asarray(rng.standard_normal(shape, dtype=np.float32)).astype(jnp.bfloat16)
    ...
    out = {"n_largest": n_largest, "bf16_largest": 2 * n_largest, "int8_largest": n_largest,
    ...
    for key, li, i, k2d, wspec, sspec, block, name in list(params(model)):
        ...
        params_dict[key] = param  # JaxAutoWeightsLoader.load_weights: params_dict = dict(named_parameters())
    ...
    for key, li, i, k2d, wspec, sspec, block, name in list(params(model)):
        ...
        with jax.set_mesh(mesh):
            ...
            try:
                ...
                pass
        delattr(block, name)
        setattr(block, name, nnx.Param(w_q))  # the old Param lives on in params_dict
        scales[key] = nnx.Param(w_s)
        old_q_phys += shards(w_q) + shards(w_s)
        del weight, w2d, w_q, w_s
        peak = max(peak, phys(mesh_devs) - base)
    jnp.clip = orig_clip
    out["before"] = {"after_stream": after_stream, "peak_at_clip": peak_clip, "peak": max(peak, peak_clip),
                     "end": phys(mesh_devs) - base, "bf16": bf16_phys, "quantized": old_q_phys}
    for key, li, i, k2d, wspec, sspec, block, name in list(params(model)):
        old[key] = (np.asarray(getattr(block, name)[...]).tobytes(), np.asarray(scales[key][...]).tobytes())
    ...
    model = abstract_model()
    for key, li, i, k2d, wspec, sspec, block, name in list(params(model)):
        h.request_host_quant(getattr(block, name), h.HostQuantRequest(jnp.int8, k2d, wspec, sspec))
    ...
    for key, li, i, k2d, wspec, sspec, block, name in list(params(model)):
        ...
        w = checkpoint(li, i)
        with jax.set_mesh(mesh):
            wu.assign_and_shard_param(param, w, key, mesh=mesh)  # REAL: host path
        del w
        assert param.get_metadata()["_is_loaded"] is True
        placed = param[...]
        assert placed.dtype == jnp.int8 and placed.shape == k2d, (key, placed.dtype, placed.shape)
        assert placed.sharding.spec == P(*wspec), (key, placed.sharding.spec, wspec)
        w_s = h.adopt_host_quant_scale(param)
        assert w_s is not None and w_s.dtype == jnp.float32 and w_s.sharding.spec == P(*sspec), key
    ...
    out["after"] = {"peak": peak, "end": phys(mesh_devs) - base, "quantized": new_q_phys, "host_peak": host_peak}
    for key, li, i, k2d, wspec, sspec, block, name in list(params(model)):
        new[key] = (np.asarray(getattr(block, name)[...]).tobytes(), np.asarray(scales[key][...]).tobytes())

    # ------------------------------------------------------------ numerics
    eager = {}
    for key, li, i, k2d, *_ in list(params(model)):
        with jax.set_mesh(host_mesh):
            e_q, e_s = q.quantize_tensor(jnp.int8, checkpoint(li, i).reshape(k2d), axis=0)
    ...
    with jax.set_mesh(mesh):
        wu.assign_and_shard_param(plain, checkpoint(0, 0), "plain", mesh=mesh)  # no request
    h.request_host_quant(req_on_mesh, h.HostQuantRequest(jnp.int8, (256, 64), ("model", None), (None, )))
    with jax.set_mesh(mesh):
        already_there = jnp.asarray(np.ones((256, 64), np.float32)).astype(jnp.bfloat16)  # born ON the mesh
        wu.assign_and_shard_param(req_on_mesh, already_there, "on_mesh", mesh=mesh)
    out["controls"] = {
        "no_request_stays_bf16": str(plain[...].dtype),
        "on_mesh_request_stays_bf16": str(req_on_mesh[...].dtype),
        "on_mesh_request_parks_no_scale": h.adopt_host_quant_scale(req_on_mesh) is None,
    }
    return out
...
def _result():
    ...
    env = dict(os.environ)
    env["XLA_FLAGS"] = (env.get("XLA_FLAGS", "") + f" --xla_force_host_platform_device_count={N_DEVICES}").strip()
    env["JAX_PLATFORMS"] = "cpu"
    r = subprocess.run([sys.executable, __file__, "--measure"], env=env, capture_output=True, text=True,
...
def test_after_the_fix_the_mesh_only_ever_holds_quantized_kernels():
    ...
    assert a["end"] < r["before"]["bf16"], "the quantized model must be smaller than the bf16 one it replaces"
...
def test_before_the_fix_the_whole_bf16_model_stayed_resident_plus_two_f32_temporaries():
    ...
    r = _result()
    b = r["before"]
    assert b["after_stream"] == b["bf16"], "every kernel should be bf16-resident after the stream"
...
def test_host_quantized_kernels_are_bit_identical_to_the_device_path():
    ...
    assert n["host_equals_old_device_path"], "int8 codes/scales differ from the pre-fix on-mesh requant"
    assert n["host_equals_eager_on_host"], "int8 codes/scales differ from eager quantize_tensor on the host"


def test_negative_controls_take_the_plain_path():
    r = _result()
    c = r["controls"]
    assert c["no_request_stays_bf16"] == "bfloat16", "a Param without a request must be placed as-is"
