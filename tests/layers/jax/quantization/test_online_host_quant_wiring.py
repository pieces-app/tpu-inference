...
def test_assign_and_shard_param_tries_the_host_hook_before_placing_bf16():
    ...
    assert body.index("place_host_quantized(") < body.index(
        "set_value(shard_put("
...
def test_every_loader_path_ends_in_assign_and_shard_param():
    ...
    assert "assign_and_shard_param(jax_param, jax_weight" in _fn(
        WU, "load_nnx_param_from_reshaped_torch")
    assert "assign_and_shard_param(param, fused" in _fn(
        UNQ, "_load_merged_tensor", "UnquantizedMergedLinearMethod")
    assert "assign_and_shard_param(param, dummy_weight" in _fn(
        WU, "load_weights", "JaxDummyModelLoader")
...
def test_the_request_rides_on_param_metadata_not_on_the_layer():
    ...
    assert names == [
        "dtype", "kernel_shape", "weight_spec", "scale_spec", "axis"
    ], names
...
def test_engagement_marker_is_emitted_on_both_paths():
    ...
    ) == 2, "both the host path and the fallback must announce"
...
def test_the_leaf_imports_only_jax_and_the_quantization_leaf():
    ...
    for n in tree.body:
        if isinstance(n, ast.Import):
            ...
            mods.add(n.module)
    allowed = {
        "dataclasses", "typing", "jax", "jax.sharding",
...
def test_the_leaf_computes_under_the_arrays_own_mesh():
    ...
    calls = {
        n.func.id
        for n in ast.walk(ast.parse(LEAF.read_text()))
