...
def _case(seed=0, b=2, p=6, patch_dim=12, mm=8, S=5):
    ...
    P = dict(ln1_w=rng.normal(1, .1, patch_dim), ln1_b=rng.normal(0, .1, patch_dim),
             dense_w=rng.normal(0, .3, (patch_dim, mm)), dense_b=rng.normal(0, .1, mm),
             ln2_w=rng.normal(1, .1, mm), ln2_b=rng.normal(0, .1, mm),
             pos_embedding=rng.normal(0, .5, (S, 2, mm)),
             pos_norm_w=rng.normal(1, .1, mm), pos_norm_b=rng.normal(0, .1, mm))
...
def test_vision_embedder_matches_independent_reference():
    ...
    m = _math()
    P, pv, pos = _case()
    got = np.asarray(m.unified_vision_embed(jnp.asarray(pv), jnp.asarray(pos),
...
def _load_weights_src():
    src = MODEL.read_text(); tree = ast.parse(src)
    for c in ast.walk(tree):
        if isinstance(c, ast.ClassDef) and c.name == "Gemma4UnifiedForConditionalGeneration":
            for f in c.body:
...
def test_both_placeholder_ids_are_merged():
    src = MODEL.read_text()
    i = src.index("def embed_input_ids"); j = src.index("def get_single_image_embedding")
    body = src[i:j]
...
def test_layernorm_eps_is_torch_default_not_rms_eps():
    ...
    i = src.index("class Gemma4UnifiedVisionEmbedder"); j = src.index("class Gemma4UnifiedModel")
    emb = src[i:j]
    assert emb.count("epsilon=_LAYERNORM_EPS") == 3, "three LayerNorms, all torch-default eps"
...
def test_embedder_op_order():
    ...
    order = [call.index(t) for t in ("self.patch_ln1(", "self.patch_dense(", "self.patch_ln2(",
                                     "factorized_posemb(", "self.pos_norm(")]
    assert order == sorted(order), "op order must be ln1 -> dense -> ln2 -> +posemb -> pos_norm"


def test_multimodal_front_end_is_not_quantized():
    src = MODEL.read_text()
    i = src.index("class Gemma4UnifiedVisionEmbedder"); j = src.index("class Gemma4UnifiedForConditionalGeneration")
...
def test_every_checkpoint_tensor_maps_onto_the_module_tree():
    h = json.loads(FIXTURE.read_text()); h.pop("__metadata__", None)
    assert len(h) == 677
    ...
    assert not any(any(s in k for s in skip) for k in h), "fixture has quant stats?"
    non_text = sorted(k for k in h if not k.startswith("model.language_model."))
    assert non_text == [
